...
def select_action(policy_model, state):
    ...
    curpolicy = _get_all_policies(attack_id_list, espilon_id_list, attack_iters_id_list, step_size_id_list,
                                  beta_id_list, args)
...
def train_target_model(input_batch, delta, y_batch, copy_target_model, beta_batch):
    ...
    all_loss_robust = beta_batch * torch.sum(
        criterion_kl_not_sum(F.log_softmax(copy_target_model(X + delta), dim=1),
                             F.softmax(copy_target_model(X), dim=1)), dim=1)
...
def Get_reward(input_batch, y_batch, target_model, action):
    ...
    all_loss_robust = beta_batch * torch.sum(
        criterion_kl_not_sum(F.log_softmax(output, dim=1), F.softmax(ori_clean_output, dim=1)), dim=1)
    # print(all_loss_robust.shape)
...
criterion = nn.CrossEntropyLoss()
target_model_optimizer = optim.SGD(target_model.parameters(), lr=args.target_model_lr, momentum=0.9,
...
if args.target_model_lr_scheduler == 'cyclic':
    ...
    target_model_scheduler = torch.optim.lr_scheduler.MultiStepLR(target_model_optimizer,
                                                                  milestones=[int(lr_steps * 75 / 100),
                                                                              int(lr_steps * 90 / 100)],
                                                                  gamma=0.1)
