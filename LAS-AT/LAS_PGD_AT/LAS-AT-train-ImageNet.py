...
criterion = nn.CrossEntropyLoss()
target_model_optimizer = optim.SGD([{'params': target_model.parameters(), 'initial_lr': args.target_model_lr}],
                                   lr=args.target_model_lr, momentum=0.9, weight_decay=5e-4)
...
if args.target_model_lr_scheduler == 'cyclic':
    ...
    target_model_scheduler = torch.optim.lr_scheduler.MultiStepLR(target_model_optimizer,
                                                                  milestones=[int(lr_steps * 99 / 110),
                                                                              int(lr_steps * 104 / 110)],
                                                                  gamma=0.1)
